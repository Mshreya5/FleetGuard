...
//   validateDates,
// };
const INDIAN_VEHICLE_NUM_REGEX =
...
const validateVehicleData = (data:any, isUpdate:boolean = false) => {
    ...
    if (registrationNumber || !isUpdate) {
        ...
        if (
            ...
            if (!INDIAN_VEHICLE_NUM_REGEX.test(cleanReg)) {
                errors.push(
                    `Invalid Indian vehicle registration number format (${cleanReg}). Must be uppercase without hyphens or spaces. Example: KA01AB1234, MH12DE1234`
                );
            }
        }
    }
    ...
    if (model || !isUpdate) {
        ...
        if (
            ...
            if (
                cleanModel.length < 2 ||
                cleanModel.length > 50
            ) {
                errors.push(
                    "Vehicle model must be between 2 and 50 characters"
                );
            }

            if (/^\d+$/.test(cleanModel)) {
                errors.push(
                    "Vehicle model cannot contain numbers only"
                );
            }
        }
    }
    ...
    if (
        ...
        if (
            ...
            errors.push(
                `Manufacturing year must be a valid number between 1980 and ${currentYear}`
            );
        }
    }
    ...
    if (
        ...
        if (
            ...
            errors.push(
                "Mileage must be a positive number up to 2,000,000 km"
            );
        }
    }
    ...
    if (vin) {
        ...
        const cleanVin:string =
            vin.trim().toUpperCase();

        if (!VIN_REGEX.test(cleanVin)) {
            errors.push(
                "VIN must be exactly 17 uppercase alphanumeric characters (excluding I, O, Q)"
            );
        }
    }
...
const validateUserData = (
    ...
    if (name || !isUpdate) {
        ...
        if (
            ...
            errors.push(
                "Name must contain only alphabets and spaces, between 2 and 50 characters"
            );
        }
    }
    ...
    if (email || !isUpdate) {
        ...
        if (
            ...
            errors.push(
                "Please enter a valid RFC-compliant email address (e.g. user@domain.com)"
            );
        }
    }
    ...
    if (licenseNumber) {
        ...
        if (
            ...
            errors.push(
                "License number format is invalid (expected 8-20 alphanumeric characters/hyphens)"
            );
        }
    }
...
const validateDates = ({
    ...
    if (expiryDate && !isHistorical) {
        ...
        if (expiry < now) {
            ...
            errors.push(
                "Insurance/compliance document cannot be already expired when creating new record"
            );
        }
    }
