...
const getDriverId = (req:any) =>
    req?.user?.id ||
    req?.user?._id ||
    "driver-001";

const getDriverName = (req:any) =>
    req?.user?.name ||
    "Driver";

const getDriverQuery = (req:any) => {

    const driverId:string = getDriverId(req);
    const driverName:string = getDriverName(req);
...
const buildDriverPayload = async (req:any) => {
    ...
    let assignment =
        await Assignment.findOne(driverQuery)
            .sort({ assignedDate: -1 })
            .lean();

    let assignments =
        await Assignment.find(driverQuery)
            .sort({ assignedDate: -1 })
            .lean();
    ...
    return {
        ...
        tripStatus,
        complianceStatus:
            assignment?.complianceStatus || "Valid",
        pendingChecklistCount:
            checklist?.status === "Completed" ? 0 : 6
    };
};
...
const createChecklist = async (
    ...
    try {
        ...
        if (
            typeof tyres !== "boolean" ||
            typeof brakes !== "boolean" ||
            typeof lights !== "boolean" ||
            typeof fuel !== "boolean" ||
            typeof mirrors !== "boolean" ||
            typeof horn !== "boolean"
        ) {

            return res.status(400).json({
                message:
                    "All checklist fields are required"
            });
        }
...
const startTrip = async (
    ...
    try {
        ...
        if (
            !trip ||
            !trip.checklistCompleted
        ) {

            return res.status(400).json({

                message:
                    "Checklist must be completed before starting trip"
            });
        }
    ...
    try {
        ...
        if (
            ...
            return res.status(400).json({
                ...
                message:
                    "All issue report fields are required"
            });
        }
...
const getServiceHistory = async (
    ...
    try {
        ...
        const serviceHistory =
            await ServiceHistory.find(driverQuery)
                .sort({ performedDate: -1 })
                .lean();
