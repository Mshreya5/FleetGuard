...
import { validateVehicleData } from "../middleware/validation";
...
const recalculateComplianceStatus = async (
    vehicleId:any
) => {
    ...
    try {
        ...
        let insuranceExpiry:any =
            null;


        let pollutionStatus:string =
            "Missing";

        let pollutionExpiry:any =
            null;
    ...
    try {
        ...
        if (existingReg) {
            ...
            return res.status(409).json({
                ...
                message:
                    `Vehicle with registration number '${formattedReg}' already exists`
            });
        }
        ...
        if (vin) {
            ...
            if (existingVin) {
                ...
                return res.status(409).json({
                    ...
                    message:
                        `Vehicle with VIN '${cleanVin}' already exists`
                });
            }
        }
        ...
        await logAudit({
            ...
            next:
                `${formattedReg} (${model}) added to fleet`,
...
const updateVehicle = async (
    ...
    try {
        ...
        const prevVal:string =
            `${vehicle.registrationNumber} (${vehicle.model}, ${vehicle.status})`;
        ...
        if (
            ...
            if (existingReg) {
                ...
                return res.status(409).json({
                    ...
                    message:
                        `Registration number '${formattedReg}' already in use`
                });
            }
        ...
        await logAudit({
            ...
            next:
                `${updatedVehicle.registrationNumber} (${updatedVehicle.model}, ${updatedVehicle.status})`,

            reason:
                "Vehicle specifications or status modified"
        });
...
const deleteVehicle = async (
    ...
    try {
        ...
        await logAudit({
            ...
            reason:
                "Vehicle decommissioned/deleted"
        });
        ...
        res.status(200).json({
            ...
            message:
                `Vehicle ${regNum} and associated assignments/compliance records deleted successfully`
        });
...
const getMaintenanceCount = async (
    req:any,
    res:any
) => {
    ...
    try {
        ...
        const ServiceQueue =
            require("../models/ServiceQueue");


        const count =
            await ServiceQueue.countDocuments({
