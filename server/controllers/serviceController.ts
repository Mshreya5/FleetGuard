...
const getServiceDashboard = async (req: any, res: any) => {
  try {
    ...
    const totalRevenue = completedServices.reduce((sum: any, item: any) => sum + (Number(item.cost) || 0), 0);
    ...
    res.status(200).json({
      ...
      upcomingServices: upcomingServices.map((s: any) => ({
        id: s._id,
...
// GET QUEUE ITEMS
const getServiceQueue = async (req: any, res: any) => {
  try {
...
const createServiceRequest = async (req: any, res: any) => {
  try {
...
const completeService = async (req: any, res: any) => {
  try {
    const { id } = req.params;
...
const updateVehicleMileage = async (req: any, res: any) => {
  try {
