...
import { recalculateComplianceStatus } from "./vehicleController";
    ...
    try {
        ...
        if (
            ...
            return res.status(400).json({
                ...
                message:
                    "Vehicle number and issue description are required"
            });
        }
...
// COMPLETE SERVICE
// Cross-Module Sync: Updates Vehicle History, Mileage & Operational Status
...
const completeService = async (
    ...
    try {
        ...
        const queueItem =
            await ServiceQueue.findById(id);
        ...
        const history =
            new ServiceHistory({
                ...
                mileageAtService:
                    updatedMileage
                        ? Number(updatedMileage)
                        : queueItem.currentMileage,
        ...
        await logAudit({
            ...
            reason:
                `Service completed for ${reg}. Cost: $${finalCost}`
        });
        ...
        res.status(200).json({
            ...
            message:
                `Service completed for vehicle ${reg}. Vehicle set back to Operational.`,
    ...
    try {
        ...
        if (
            ...
            return res.status(400).json({
                ...
                message:
                    "Vehicle number and current mileage are required"
            });
        }


        const newMileage:number =
        ...
        if (
            newMileage >=
            (vehicle.serviceDueMileage || 10000)
        ) {
        ...
        res.status(200).json({
            ...
            message:
                `Mileage updated for ${reg} from ${prevMileage} km to ${newMileage} km`,
