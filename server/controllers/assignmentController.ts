...
const assignVehicle = async (req:any, res:any) => {
    try {
        ...
        if (
            ...
            return res.status(400).json({
                success: false,
                message: "Vehicle and Driver details are required"
            });
        }
        ...
        const finalDriverName:string =
            driver
                ? driver.name
                : (driverName || "Driver").trim();

        const finalDriverId:any =
            driver
                ? driver._id
                : (driverId || "driver-001");

        // Business Rule Check:
        // Non-compliant vehicle assignment
        const complianceStatus:string =
            vehicle.complianceSummary?.overallStatus || "Valid";
        ...
        await logAudit({
            ...
            status: "Success",
            next:
                `Assigned ${vehicle.registrationNumber} to ${finalDriverName}`,
            reason:
                isOverrideApplied
                    ? `Assigned with override: ${overrideReason}`
                    : "Standard vehicle assignment"
        });

        res.status(201).json({
            success: true,
            message:
                `Vehicle ${vehicle.registrationNumber} successfully assigned to ${finalDriverName}`,
            assignment,
            vehicle
        });
...
const unassignVehicle = async (req:any, res:any) => {
    try {
        ...
        const activeAssignment =
            await Assignment.findOne(query);

        if (!activeAssignment) {
            return res.status(404).json({
                success: false,
                message:
                    "No active assignment found for this vehicle or driver"
            });
        }
        ...
        await Vehicle.updateOne(
            {
                $or: [
                    {
                        _id: activeAssignment.vehicleId
                    },
                    {
                        registrationNumber:
                            activeAssignment.registrationNumber
                    }
                ]
            },
            {
        ...
        await logAudit({
            ...
            status: "Success",
            prev:
                `Assigned to ${activeAssignment.driverName}`,
            next:
                "Unassigned (Available)",
            reason:
                "Assignment unassigned/completed"
        });

        res.status(200).json({
            success: true,
            message:
                `Vehicle ${activeAssignment.registrationNumber} unassigned successfully`
        });
...
const getAssignments = async (req:any, res:any) => {
    try {
        ...
        if (driverName) {
            query.driverName =
                new RegExp(driverName, "i");
        }

        if (vehicleNumber) {
            query.registrationNumber =
                new RegExp(vehicleNumber, "i");
        }
...
// GET ASSIGNMENTS BY DRIVER ID
const getAssignmentsByDriverId =
