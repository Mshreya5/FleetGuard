...
const assignVehicle = async (req: any, res: any) => {
  try {
    ...
    if (!vehicle && (registrationNumber || vehicleId)) {
      const reg: string = (registrationNumber || vehicleId).toUpperCase().trim();
    ...
    const finalDriverName: string = driver ? driver.name : (driverName || 'Driver').trim();
    const finalDriverId: any = driver ? driver._id : (driverId || 'driver-001');

    // Recalculate Compliance Status for Vehicle
    await recalculateComplianceStatus(vehicle._id);
    const updatedVehicle: any = (await Vehicle.findById(vehicle._id)) || vehicle;

    // Business Rule Check: Non-compliant vehicle assignment
    const complianceStatus: string = updatedVehicle.complianceStatus || updatedVehicle.complianceSummary?.overallStatus || 'Valid';
    let isOverrideApplied: boolean = false;
    ...
    if (complianceStatus === 'Expired') {
      ...
      const overrideLog: any = new OverrideLog({
        ...
        overriddenBy: (req as any).user?.name || 'Fleet Manager',
        role: (req as any).user?.role || 'Fleet Manager',
        timestamp: new Date(),
      });
      await overrideLog.save();

      await logAudit({
        user: (req as any).user?.name || 'Fleet Manager',
    ...
    const assignment: any = new Assignment({
      vehicleId: vehicle._id,
      registrationNumber: vehicle.registrationNumber,
      vehicleNumber: vehicle.registrationNumber,
      driverId: finalDriverId,
      ...
      assignedBy: (req as any).user?.name || 'Fleet Manager',
      complianceStatus,
    });
...
const unassignVehicle = async (req: any, res: any) => {
  try {
    ...
    const activeAssignment: any = await Assignment.findOne(query);
    if (!activeAssignment) {
...
const getAssignments = async (req: any, res: any) => {
  try {
    ...
    if (driverName) {
      query.driverName = new RegExp(driverName as string, 'i');
    }
    if (vehicleNumber) {
      query.registrationNumber = new RegExp(vehicleNumber as string, 'i');
    }

    const assignments: any[] = await Assignment.find(query).sort({ assignedDate: -1 });
...
const getAssignmentsByDriverId = async (req: any, res: any) => {
  try {
    const { driverId } = req.params;
    let query: any = {
      $or: [
        { driverId },
        { driverName: new RegExp(driverId as string, 'i') }
      ]
    };
    if (mongoose.Types.ObjectId.isValid(driverId)) {
      query.$or.push({ driverId: new mongoose.Types.ObjectId(driverId) });
    }
    let assignments: any[] = await Assignment.find(query).sort({ assignedDate: -1 });
