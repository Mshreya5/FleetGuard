...
const connectDB = async () => {
    ...
    let uri =
        ...
        "mongodb+srv://USERNAME:PASSWORD@cluster0.mongodb.net/fleetguard";
    ...
    try {
        ...
        console.log(
            `[FleetGuard Unified Backend] MongoDB Connected: ${conn.connection.host} / DB: ${conn.connection.name}`
        );
        ...
        conn.connection.on("connected", () => {
            console.log(
                "[FleetGuard Unified Backend] MongoDB connection established"
            );
        });

        conn.connection.on("disconnected", () => {
            console.warn(
                "[FleetGuard Unified Backend] MongoDB disconnected"
            );
        });

        conn.connection.on("error", (err:any) => {
            console.error(
                "[FleetGuard Unified Backend] MongoDB connection error:",
                err.message
            );
        });
        ...
        try {
            ...
            const vehColl =
                conn.connection.collection("vehicles");

            const idxs =
                await vehColl.indexes().catch(() => []);
        ...
        console.warn(
            `[FleetGuard Unified Backend] Primary MongoDB Atlas warning: ${error.message}. Attempting local fallback...`
        );
        ...
        try {
            ...
            const fallbackConn =
                await mongoose.connect(
                    "mongodb://127.0.0.1:27017/fleetguard",
                    {
                        dbName: "fleetguard",
                        serverSelectionTimeoutMS: 30000,
                        connectTimeoutMS: 30000,
            ...
            console.log(
                `[FleetGuard Unified Backend] Connected to local MongoDB fallback: ${fallbackConn.connection.host}`
            );
            ...
            console.error(
                `[FleetGuard Unified Backend] MongoDB Connection Error: ${fallbackError.message}`
            );
