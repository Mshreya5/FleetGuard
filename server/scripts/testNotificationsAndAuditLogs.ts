...
async function testNotificationsAndAudit(): Promise<void> {
    console.log("====================================================");
    console.log("RUNNING NOTIFICATION & AUDIT LOG SYSTEM VERIFICATION");
    console.log("====================================================");
    ...
    try {
        ...
        console.log(
            `✅ [PASS] MongoDB has ${totalNotifs} notifications (Unread: ${unreadCount})`
        );

        // 3. Query Role-Based Notifications
        const driverNotifs = await Notification.find({
            role: { $in: ["Driver", "All"] }
        });

        console.log(
            `✅ [PASS] Driver role notifications query returned ${driverNotifs.length} items`
        );

        const fmNotifs = await Notification.find({
            role: { $in: ["Fleet Manager", "Admin", "All"] }
        });

        console.log(
            `✅ [PASS] Fleet Manager role notifications query returned ${fmNotifs.length} items`
        );
        ...
        if (sampleNotif) {
            ...
            console.log(
                `✅ [PASS] Mark as read updated MongoDB unread count to ${updatedUnread}`
            );
        }
        ...
        console.log(
            `✅ [PASS] MongoDB Audit Logs total: ${totalLogs}, Today: ${todayLogs}`
        );
