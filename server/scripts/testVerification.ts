...
async function runTests(): Promise<void> {
    ...
    validRegs.forEach((reg: string) => {
        const errs = validateVehicleData({
        ...
        assert(
            errs.length === 0,
            `Valid Reg Number: ${reg}`
        );
    });

    invalidRegs.forEach((reg: string) => {
        const errs = validateVehicleData({
            registrationNumber: reg,
            model: "XUV700",
            manufacturingYear: 2022,
    ...
    assert(
        ...
        "Reject numbers-only model name"
    );
    ...
    assert(
        invalidUserErrs.length === 4,
        "Reject invalid name, email, phone, and password complexity"
    );

    // TEST 5: Dates
    const invalidDateErrs = validateDates({
        issueDate: "2026-10-10",
        expiryDate: "2025-10-10"
    });

    assert(
        invalidDateErrs.length > 0,
        "Reject expiry date before issue date"
    );
    ...
    assert(
        userCount > 0,
        `Database has ${userCount} active users`
    );

    assert(
        vehicleCount > 0,
        `Database has ${vehicleCount} active vehicles`
    );

    assert(
        auditCount > 0,
        `Database has ${auditCount} persistent audit logs`
    );
