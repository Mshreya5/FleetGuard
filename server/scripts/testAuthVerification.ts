...
const connectDB = async (): Promise<void> => {
    const connStr =
        ...
        "mongodb://localhost:27017/fleetguard";
...
const runAuthVerification = async (): Promise<void> => {
    let passed: number = 0;
    let failed: number = 0;

    const test = (title: string, condition: boolean): void => {
        if (condition) {
            console.log(`✅ [PASS] ${title}`);
            passed++;
        } else {
    ...
    // 1. Validation Regex Tests
    test(
        "Valid Name: Alex Johnson",
        NAME_REGEX.test("Alex Johnson")
    );

    test(
        "Valid Name: Kiran Shree",
        NAME_REGEX.test("Kiran Shree")
    );

    test(
        "Reject Invalid Name with Numbers: Alex123",
        !NAME_REGEX.test("Alex123")
    );

    test(
        "Reject Invalid Name with Special Chars: Alex@John",
        !NAME_REGEX.test("Alex@John")
    );

    test(
        "Valid RFC Email: test.user@fleetguard.com",
        EMAIL_REGEX.test("test.user@fleetguard.com")
    );

    test(
        "Reject Invalid Email: user@com",
        !EMAIL_REGEX.test("user@com")
    );

    test(
        "Reject Invalid Email: userfleetguard.com",
        !EMAIL_REGEX.test("userfleetguard.com")
    );

    test(
        "Valid Strong Password: Password@123",
        PASSWORD_REGEX.test("Password@123")
    );

    test(
        "Reject Weak Password (No Special Char): Password123",
        !PASSWORD_REGEX.test("Password123")
    );

    test(
        "Reject Weak Password (<8 Chars): Pass@1",
        !PASSWORD_REGEX.test("Pass@1")
    );
    ...
    test(
        "User successfully registered & stored in MongoDB",
        Boolean(newUser._id)
    );

    test(
        "Password is saved hashed with bcrypt",
        newUser.password.startsWith("$2a$") ||
        newUser.password.startsWith("$2b$")
    );
    ...
    test(
        "Profile update saves name, phone, address to MongoDB",
        refreshedUser !== null &&
        refreshedUser.name === "Updated Verification User" &&
        refreshedUser.phone === "9876543210"
    );
...
runAuthVerification().catch((err:any) => {
    console.error("Test Execution Error:", err);
    process.exit(1);
});