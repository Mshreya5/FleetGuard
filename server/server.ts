...
//     if (!dbConnection) {
//       console.error('[FleetGuard Unified Backend] Failed to connect to database');
//       process.exit(1);
//     }
//     app.listen(PORT, () => {
//       console.log(`====================================================`);
...
import {
    getServiceCosts,
    createServiceCost,
    getServiceHistory as getExtensionsHistory,
    createServiceHistory as createExtensionsHistory,
    getServiceSchedules,
    createServiceSchedule,
    getMaintenanceRisk,
    completeServiceExtension
} from "./controllers/serviceExtensionsController";
...
import adminRoutes from "../src/modules/Admin/backend/src/routes/adminRoutes";
import fmComplianceRoutes from "../src/modules/FleetManager/backend/routes/complianceRoutes";
import fmDashboardRoutes from "../src/modules/FleetManager/backend/routes/dashboardRoutes";
...
const uploadsPath = path.join(
    __dirname,
    "../src/modules/FleetManager/backend/uploads"
);
...
app.get(
    "/api/admin/fleet-managers/count",
    verifyToken,
    requireRole(["Admin"]),
    getFleetManagerCount
);
...
app.post(
    "/api/vehicles",
    verifyToken,
    requireRole(["Admin", "Fleet Manager"]),
    addVehicle
);
...
app.get(
    "/api/admin/vehicles/maintenance-count",
    verifyToken,
    requireRole(["Admin"]),
    getMaintenanceCount
);
...
app.use(
    "/api/v1/fleet-manager/assignments",
    verifyToken,
    getAssignments
);
...
app.use(
    "/api/v1/fleet-manager/compliance",
    fmComplianceRoutes
);
...
app.get(
    "/api/service-center/dashboard",
    getServiceDashboard
);

app.get(
    "/api/service-center/queue",
    getServiceQueue
);

app.post(
    "/api/service-center/requests",
    verifyToken,
    requireRole([
        "Admin",
        "Fleet Manager",
...
app.post(
    "/api/service-center/complete/:id",
    verifyToken,
    requireRole([
        "Admin",
        "Service Center"
...
app.get(
    "/api/service-center/extensions/costs",
    getServiceCosts
);
...
app.get(
    "/api/service-center/extensions/history",
    getExtensionsHistory
);

app.post(
    "/api/service-center/extensions/history",
    createExtensionsHistory
);

app.get(
    "/api/service-center/extensions/historical",
    getServiceCosts
);

app.get(
    "/api/service-center/extensions/schedules",
    getServiceSchedules
);
...
app.get(
    "/api/service-center/extensions/risk",
    getMaintenanceRisk
);

app.post(
    "/api/service-center/extensions/complete",
    completeServiceExtension
);
...
app.use(
    (err: any, req: any, res: any, next: any) => {
        ...
        console.error(
            "[FleetGuard Unified Backend Error]:",
            err.message || err
        );
...
async function startServer(): Promise<void> {
    ...
    try {
        ...
        if (!dbConnection) {
            ...
            console.error(
                "[FleetGuard Unified Backend] Failed to connect to database"
            );
        ...
        app.listen(
            ...
            () => {
                ...
                console.log(
                    `[FleetGuard Unified Server] Running on http://localhost:${PORT}`
                );
                ...
                console.log(
                    "- Service Center:    /api/service-center"
                );
        ...
        console.error(
            "[FleetGuard Unified Backend] Server startup error:",
            error.message
        );
