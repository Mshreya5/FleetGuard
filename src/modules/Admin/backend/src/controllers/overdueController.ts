...
const getOverdueCompliance = async (req: any, res: any) => {
  try {
    ...
    if (search) {
      ...
      filtered = filtered.filter((r: any) => r.registrationNumber.toLowerCase().includes(term));
    }

    if (filter !== 'all') {
      filtered = filtered.filter((r: any) => r.documentType === filter);
    }
