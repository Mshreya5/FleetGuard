...
const getDocumentStatus = (daysRemaining: any) => {
...
const buildComplianceData = (vehicles: any) => {
  ...
  const compliantCount = transformed.filter((vehicle: any) => vehicle.overall === 'Compliant').length;
  const expiredInsurance = transformed.filter((vehicle: any) => vehicle.insurance === 'Expired').length;
  const expiredPollution = transformed.filter((vehicle: any) => vehicle.pollution === 'Expired').length;
  const expiredFitness = transformed.filter((vehicle: any) => vehicle.fitness === 'Expired').length;
...
const buildExpiryData = (vehicles: any) => {
  const expiryRecords: any[] = [];
...
const getDashboardData = async (req: any, res: any) => {
  try {
    ...
    const vehiclesUnderMaintenance = vehicles.filter((vehicle: any) =>
    ...
    const upcomingExpiriesCount = expiryRecords.filter((r: any) => r.status === 'Expiring Soon' || r.status === 'Expired').length;
    ...
    const notifications = realNotifications.map((n: any) => n.message || n.description || n.title);
...
const getComplianceData = async (req: any, res: any) => {
  try {
    const vehicles = await Vehicle.find({}).lean().catch(() => []);
    const complianceData = buildComplianceData(vehicles);
...
const getUpcomingExpiryData = async (req: any, res: any) => {
  try {
    const vehicles = await Vehicle.find({}).lean().catch(() => []);
