...
const getFleetReport = async (req: any, res: any) => {
  try {
    ...
    const inactiveVehicles = vehicles.filter((v: any) => v.status !== 'Active').length;

    const assignments = vehicles.map((v: any) => ({
    ...
    const totalServiceCost = maintenance.reduce((sum: any, r: any) => sum + r.cost, 0);
