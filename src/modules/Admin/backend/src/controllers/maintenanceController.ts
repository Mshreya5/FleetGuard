...
const getServiceCostSummary = async (req: any, res: any) => {
  try {
    const records = await Maintenance.find({}).lean();

    const totalCost = records.reduce((sum: any, r: any) => sum + r.cost, 0);

    const now = new Date();
    const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
    const monthlyCost = records
      .filter((r: any) => new Date(r.serviceDate) >= monthStart)
      .reduce((sum: any, r: any) => sum + r.cost, 0);

    const uniqueVehicles = new Set(records.map((r: any) => r.vehicle)).size;
...
const getAllMaintenance = async (req: any, res: any) => {
  try {
