...
const getVehicle = async (req, res) => {
    try {
        ...
        const complianceDocs = await Compliance.find({ vehicleId: vehicle._id }).sort({ createdAt: -1 });
        const assignmentHistory = await Assignment.find({ vehicleId: vehicle._id }).sort({ createdAt: -1 });
...
const updateVehicle = async (req, res) => {
    try {
        ...
        if (req.body.registrationNumber && req.body.registrationNumber.toUpperCase().trim() !== vehicle.registrationNumber) {
            const dup = await Vehicle.findOne({ registrationNumber: req.body.registrationNumber.toUpperCase().trim() });
            if (dup) {
...
const deleteVehicle = async (req, res) => {
    try {
        ...
        await Compliance.deleteMany({ vehicleId: vehicle._id });
        await Assignment.deleteMany({ vehicleId: vehicle._id });
