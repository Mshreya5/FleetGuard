...
const getComplianceStatus = async (req: any, res: any) => {
    try {
...
const getUpcomingExpiries = async (req: any, res: any) => {
    try {
        if (mongoose.connection.readyState !== 1) {
        ...
        const expiringDocs = (docs || []).filter((doc: any) => {
        ...
        expiringDocs.sort((a: any, b: any) => a.daysRemaining - b.daysRemaining);
