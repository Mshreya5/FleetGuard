      ...
      <section style={{ position: 'relative', flex: 1, display: 'flex', alignItems: 'center', overflow: 'hidden', minHeight: '100vh' }}>
        ...
        <div style={{ position: 'relative', zIndex: 10, maxWidth: '1280px', margin: '0 auto', padding: `96px ${SPACING[6]} ${SPACING[20]}`, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: SPACING[16], alignItems: 'center', width: '100%' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: SPACING[8] }}>
            ...
            <div className="fg-fade-up-1" style={{ display: 'flex', flexDirection: 'column', gap: SPACING[3] }}>
              <h1 style={{ fontSize: 'clamp(48px, 7vw, 72px)', fontWeight: FONT.weight.extrabold, lineHeight: 1.1, letterSpacing: '-1px', margin: 0 }}>
                <span className="fg-shimmer-text">Fleet</span>
            ...
            <p className="fg-fade-up-2" style={{ color: COLORS.textSecondary, fontSize: FONT.size.base, lineHeight: 1.75, maxWidth: '440px', margin: 0 }}>
            ...
            <div className="fg-fade-up-3" style={{ display: 'flex', flexWrap: 'wrap', gap: SPACING[3] }}>
              ...
              <a href="/login" style={{ display: 'inline-flex', alignItems: 'center', padding: `${SPACING[3]} ${SPACING[8]}`, borderRadius: RADIUS.btn, border: `1px solid rgba(74,144,226,0.4)`, color: COLORS.textPrimary, fontSize: FONT.size.sm, fontWeight: FONT.weight.semibold, textDecoration: 'none', transition: 'border-color 0.2s, color 0.2s' }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = COLORS.primaryLight; e.currentTarget.style.color = COLORS.primaryLight; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(74,144,226,0.4)'; e.currentTarget.style.color = COLORS.textPrimary; }}
              >
                Login
              </a>
            </div>

            <div className="fg-fade-up-3" style={{ display: 'flex', gap: SPACING[10], paddingTop: SPACING[6], borderTop: `1px solid rgba(255,255,255,0.1)` }}>
            ...
            <div className="fg-float" style={{ width: '100%', maxWidth: '420px', background: 'rgba(13,21,38,0.82)', backdropFilter: 'blur(20px)', border: `1px solid rgba(37,99,235,0.3)`, borderRadius: RADIUS.lg, overflow: 'hidden', boxShadow: '0 8px 40px rgba(37,99,235,0.3)' }}>
              <div style={{ padding: `${SPACING[4]} ${SPACING[5]}`, borderBottom: `1px solid ${COLORS.border}`, background: 'rgba(30,58,95,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: FONT.size.xs, fontWeight: FONT.weight.medium, color: COLORS.primaryLight, background: 'rgba(74,144,226,0.1)', border: `1px solid rgba(74,144,226,0.2)`, padding: `3px 10px`, borderRadius: RADIUS.full }}>
                  <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: COLORS.primaryLight, animation: 'pulseGlow 2s ease-in-out infinite' }} />
                  Live
                </span>
              </div>
              ...
              <div style={{ padding: `0 ${SPACING[5]} ${SPACING[5]}`, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: SPACING[3] }}>
                {metrics.map(({ icon: Icon, label, value }) => (
                  <div key={label} className="fg-metric-tile" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px', padding: SPACING[3], background: 'rgba(5,8,16,0.6)', border: `1px solid ${COLORS.border}`, borderRadius: RADIUS.btn, cursor: 'default' }}>
